...
Plot least squares polynomial fit between coreness and FC (measure as correlation)
"""
...
idx_select = np.array(
    df_stats["node_idx"][
        (df_stats["metric"] == metric) & (df_stats["p-val_corrected"] < P_VAL)
    ]
)
...
Xfc_syn_mean_non_zero = np.divide(
    Xfc_syn_sum_along_axis0, Xfc_syn_non_zero_count, where=Xfc_syn_non_zero_count != 0
)

Xfc_ctr_sum_along_axis0 = np.sum(Xfc_ctr, axis=0)
Xfc_ctr_non_zero_count = np.count_nonzero(Xfc_ctr, axis=0)
Xfc_ctr_mean_non_zero = np.divide(
    Xfc_ctr_sum_along_axis0, Xfc_ctr_non_zero_count, where=Xfc_ctr_non_zero_count != 0
)

ncols = 2
nrows = 3
for Xnet_mean, Xfc_mean, plot_name in zip(
    [Xnet_syn_mean, Xnet_ctr_mean], [Xfc_syn_mean, Xfc_ctr_mean], ["SYN", "CTR"]
):
    fig, axs = plt.subplots(nrows, ncols, figsize=(4 * ncols, 4 * nrows))
    # Loop through node_idx and populate the subplots
    for node_idx, row, col in zip(
        idx_select,
        np.sort(np.tile(np.arange(nrows), ncols)),
        ...
        # axs[row, col].plot(Xnet_mean, correlation_line(Xnet_mean),
        #                    label=f'(r = {correlation:.2f})', color='red')
        axs[row, col].plot(
            np.sort(x),
            correlation_line(np.sort(x)),
